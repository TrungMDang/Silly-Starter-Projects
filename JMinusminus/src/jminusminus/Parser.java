...
public class Parser {
	...
	private boolean seeBasicType() {
		if (see(BOOLEAN) || see(CHAR) || see(INT) || see(SHORT) || 
				see(BYTE) || see(LONG) || see(FLOAT) || see(DOUBLE)) {
			return true;
		} else {
			return false;
		}
	...
	private boolean seeReferenceType() {
		if (see(IDENTIFIER)) {
			...
			scanner.recordPosition();
			if (have(BOOLEAN) || have(CHAR) || have(INT) || have(SHORT) || 
					have(BYTE) || have(LONG) || have(FLOAT) || have(DOUBLE)) {
...
//		if (publicCount < 1) {
//			if (mods.contains(PUBLIC.image())) {
//				publicCount += 1;
//			}
//		} else {
//			isInError = true;
//			reportParserError("Error: Multiple public type declaration.");
//		}
	...
	private JMember memberDecl(ArrayList<String> mods) {
		...
		if (seeIdentLParen()) {
			...
			if (have(VOID)) {
				...
				if (have(THROWS)) {
					while(scanner.token().image() != LCURLY.image()) {									
						exceptions.add(qualifiedIdentifier());
					}
				}
				JBlock body = have(SEMI) ? null : block();
				/**Old statement that causes exception
				...
				if (seeIdentLParen()) {
					...
					JBlock body = have(SEMI) ? null : block();
					/**Old statements that causes exception
					 * memberDecl = new JMethodDeclaration(line, mods, name, type,
					 params, exceptions, body);*/
					memberDecl = new JMethodDeclaration(line, mods, name, type,
							params, body);
	...
	private JStatement statement() {
		...
		if (see(LCURLY)) {
			...
			return new JForStatement(line, control, body);
		} else if (have(SWITCH)) {
			JExpression test = parExpression();
			return new JSwitchStatement(line, test, switchBlockStatementGroups());
		} else if (have(THROW)) { 
			JExpression expr = expression();
			return new JThrowStatement(line, expr);
		} else if (have(TRY)) {
			JTryStatement block = new JTryStatement(line, blockStatement());
			ArrayList<JCatchClause> catchClauses = new ArrayList<JCatchClause>();
			if (scanner.token().image() == CATCH.image()) {
				JFinallyStatement finallyStatement = null;

				while (scanner.token().image() == CATCH.image()) {
					catchClauses.add(catchClause());
				}
				if (scanner.token().image() == FINALLY.image()) {
					mustBe(FINALLY);
					JStatement block1 = blockStatement();
					finallyStatement = new JFinallyStatement(line, block1);
				}
				return new JTryCatchFinalStatement(line, block, catchClauses, finallyStatement);
			} else {
				reportParserError("Missing catch block.");
				return block;
			}
		} else if (have(DO)) { 
			JStatement statement = blockStatement();
			if (statement != null) {
				if (see(UNTIL)) {
					mustBe(UNTIL);
					JExpression expr = expression();
					return new JDoUntilStatement(line, statement, expr);
				} else {
					mustBe(WHILE);
					JExpression expr = expression();
					return new JDoWhileStatement(line, statement, expr);
				}
				
				
			} else {
				reportParserError("Missing Do block.");
				return statement();
			}
	...
	private JCatchClause catchClause() {
		...
		mustBe(LPAREN);
		String mod = null;
		if(scanner.token().image() == FINAL.image()) {
			mod = FINAL.image();
		} 
		ArrayList<TypeName> catchType = new ArrayList<TypeName>();
		catchType.add(qualifiedIdentifier());
		if (have(BIOR))	{ //Special catch block, added in Java 7
			catchType.add(qualifiedIdentifier());
		}
		mustBe(IDENTIFIER);
		mustBe(RPAREN);
		JStatement block = blockStatement();
		return new JCatchClause(line, mod, catchType, block);
	}
	...
	private ArrayList<JSwitchStatementGroup> switchBlockStatementGroups() {
		int line = scanner.token().line();

		return switchBlockStatementGroup(line);
	}

	private ArrayList<JSwitchStatementGroup> switchBlockStatementGroup(int line) {
		ArrayList<JSwitchStatementGroup> result = new ArrayList<JSwitchStatementGroup>();
		if (have(LCURLY)) {
			while (have(CASE)) {
				TokenInfo temp = scanner.previousToken();
				JExpression expr = expression();
				if (expr != null) {
					JSwitchLabel label = new JSwitchLabel(line, temp, expr);
					mustBe(TERNARY_COLON);
					JStatement block = null;
					if (have(LCURLY)) {
						block = blockStatement();
						result.add(new JSwitchStatementGroup(line, label, block));
						mustBe(BREAK);
						mustBe(SEMI);
						mustBe(RCURLY);
					} else {
					...
					reportParserError("Missing expression for switch case at line " + line);
				}
			}	 
			while (have(DEFAULT)) {
				TokenInfo temp = scanner.previousToken();
				JSwitchLabel label = new JSwitchLabel(line, temp, null);
				mustBe(TERNARY_COLON);
				JStatement block = blockStatement();
				result.add(new JSwitchStatementGroup(line, label, block));
				mustBe(BREAK);
			...
			reportParserError("Missing braces at line " + line);
		}
		return result;
	}

	/**
	 * Parse a for control statement
	 * 
	 * ForControl:
    			ForVarControl
	...
	private JForControl forControl() {
		...
		JForControl result = null;
		mustBe(LPAREN);
		JVariableDeclaration init;
		
		//if (seeBasicType()) {
			init = localVariableDeclarationStatement(true);
		//}
//		} else {
...
//		}
		if (have(SEMI)) {
			...
			if (have(SEMI)) {
				...
				if (init != null && expr != null && update != null) {
					result = new JBasicForControl(line, init, expr, update);
				} else {
					reportParserError("Missing value: " + 
							(init == null ? init : (expr == null ? expr : (update == null ? update : "" ))));
				}
			}
		} else if (have(TERNARY_COLON)){
			JExpression expr = assignmentExpression();
			if (expr == null) {
				reportParserError("Missing right hand side of enhanced FOR control at line " + line);
			}
			result = new JEnhancedForControl(line, init, expr);
		}
		mustBe(RPAREN);	   	
		return result;
	}
	...
	/**
	 * OVERLOADED: Parse a local variable declaration statement.
	 * 
	 * <pre>
	 *   localVariableDeclarationStatement ::= type 
	 *                                           variableDeclarators 
	...
	private JVariableDeclaration localVariableDeclarationStatement(boolean haltBeforePunctuation) {
		int line = scanner.token().line();
		ArrayList<String> mods = new ArrayList<String>();
		ArrayList<JVariableDeclarator> vdecls = variableDeclarators(type());
		if (haltBeforePunctuation == false) {
			mustBe(SEMI);
		} 
		return new JVariableDeclaration(line, mods, vdecls);
	}
	...
	private JStatement statementExpression() {
		...
		if (expr instanceof JAssignment
				|| expr instanceof JPreIncrementOp || expr instanceof JPreDecrementOp 
...
//		}
	...
	private JExpression assignmentExpression() {
		int line = scanner.token().line();
		JExpression lhs = ternaryExpression(null);
		if (have(ASSIGN)) {
			return new JAssignOp(line, lhs, ternaryExpression(lhs));
		} else if (have(PLUS_ASSIGN)) {
			return new JPlusAssignOp(line, lhs, ternaryExpression(lhs));
		} else if (have(MINUS_ASSIGN)) {
			return new JMinusAssignOp(line, lhs, ternaryExpression(lhs));
		}else if (have(MULTI_ASSIGN)) {
			return new JMultiAssignOp(line, lhs, ternaryExpression(lhs));
		}else if (have(DIV_ASSIGN)) {
			return new JDivAssignOp(line, lhs, ternaryExpression(lhs));
		}else if (have(MOD_ASSIGN)) {
			return new JModAssignOp(line, lhs, ternaryExpression(lhs));
		}else if (have(BAND_ASSIGN)) {
			return new JBandAssignOp(line, lhs, ternaryExpression(lhs));
		}else if (have(BEOR_ASSIGN)) {
			return new JBeorAssignOp(line, lhs, ternaryExpression(lhs));
		}else if (have(BIOR_ASSIGN)) {
			return new JBiorAssignOp(line, lhs, ternaryExpression(lhs));
		}else if (have(LSHIFT_ASSIGN)) {
			return new JLshiftAssignOp(line, lhs, ternaryExpression(lhs));
		}else if (have(RSHIFT_ASSIGN)) {
			return new JRshiftAssignOp(line, lhs, ternaryExpression(lhs));
		}else if (have(URSHIFT_ASSIGN)) {
			return new JUrshiftAssignOp(line, lhs, ternaryExpression(lhs));
		}else {
			return lhs;
		} 
	}

	private JExpression ternaryExpression(JExpression theLhs) {
		int line = scanner.token().line();
		JExpression lhs = conditionalAndExpression();
		if (have(TERNARY_QUESTION)) {
			return new JTernaryOp(line, theLhs, lhs, JTernaryRightExpression(theLhs));
		} else {
			return lhs;
		}
	}

	private JExpression JTernaryRightExpression(JExpression lhs) {
		int line = scanner.token().line();
		JExpression rhs = conditionalAndExpression();
		if (have(TERNARY_COLON)) {
			JExpression rhs1 = conditionalAndExpression();
			return new JTernaryRightOp(line, lhs, rhs, rhs1);
		} else {
			reportParserError("Missing right hand side of ternary expression.");
	 ...
	 * 
	 * <pre>
	 *   conditionalAndExpression ::= bitWiseExpression // level 10
	 *                                  {LAND bitWiseExpression}
	 *                                | {LOR bitWiseExpression}
	 * </pre>
	 * 
	...
	private JExpression conditionalAndExpression() {
		...
		while (more) {
			if (have(LAND)) {
				...
				lhs = new JLogicalOrOp(line, lhs, bitWiseExpression());
			} else {
				more = false;
			}
		}
	...
	private JExpression bitWiseExpression() {
		...
		while (more) {
			if (have(BAND)) {
				lhs = new JBAndOp(line, lhs, equalityExpression());
			}else if (have(BEOR)) {
				lhs = new JBEOrOp(line, lhs, equalityExpression());
			}else if (have(BIOR)) { 
				lhs = new JBIOrOp(line, lhs, equalityExpression());
			}else {
				more = false;
			}
		}
	...
	private JExpression literal() {
		...
		} else if (have(TRUE_LITERAL)) {
			return new JLiteralTrue(line);
		} else if (have(FALSE_LITERAL)) {
			return new JLiteralFalse(line);
		} else if (have(NULL_LITERAL)) {
